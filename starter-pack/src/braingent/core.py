...
def is_braingent_root(path: Path) -> bool:
    return all((path / marker).exists() for marker in ROOT_MARKERS)


def search_parent_roots(start: Path) -> Iterable[Path]:
...
def resolve_repo_root(root: str | os.PathLike[str] | None = None) -> Path:
    """Resolve the target Braingent memory repo for CLI and library calls."""

    explicit = root or os.environ.get("BRAINGENT_ROOT")
    if explicit:
        candidate = Path(explicit).expanduser().resolve()
        if is_braingent_root(candidate):
            return candidate
        raise SystemExit(
            f"{candidate} is not a Braingent repo root. Expected preferences/taxonomy.yml, templates/, and workflows/."
        )

    for start in (Path.cwd(), Path(__file__).resolve()):
        for candidate in search_parent_roots(start):
            if is_braingent_root(candidate):
                return candidate

    raise SystemExit(
        "Could not find a Braingent repo root. Run from a Braingent repo or set BRAINGENT_ROOT=/path/to/repo."
    )


def set_repo_root(root: str | os.PathLike[str] | None = None) -> Path:
    """Set module-level paths for the target Braingent repo."""

    global REPO_ROOT
    global TAXONOMY_PATH
    global INDEX_DIR
    global RECORDS_JSON_PATH
    global RECORDS_COMPACT_JSON_PATH
...
REPO_ROOT = Path.cwd()
TAXONOMY_PATH = Path("preferences") / "taxonomy.yml"
...
def cmd_task_new(args: argparse.Namespace) -> int:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    today = today_iso()
    repositories = args.repository or ["repo--github--thedoublejay--braingent"]
...
def iter_template_files(root: Any | None = None, prefix: str = "") -> Iterable[tuple[str, Any]]:
    root = template_root() if root is None else root
    for child in sorted(root.iterdir(), key=lambda item: item.name):
        relative = f"{prefix}{child.name}"
        if child.is_dir():
            yield from iter_template_files(child, f"{relative}/")
        else:
...
def target_file_hash(path: Path) -> str | None:
    if not path.exists() or not path.is_file():
        return None
    return sha256_bytes(path.read_bytes())


def copy_template_file(relative: str, resource: Any, target_root: Path) -> None:
    target = target_root / relative
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_bytes(resource.read_bytes())


def target_has_user_content(path: Path) -> bool:
    if not path.exists():
        return False
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if target.exists() and target_has_user_content(target) and not args.force:
        if is_braingent_root(target):
            print(f"{target} already looks like a Braingent repo. Use `braingent update` instead.", file=sys.stderr)
        else:
            print(f"{target} is not empty. Pass --force to copy the starter template anyway.", file=sys.stderr)
        return 1

    if args.dry_run:
        print(f"Would initialize Braingent memory repo at {target}")
        print(f"Would copy {len(manifest)} starter files")
        print("Would run reindex after copy")
        return 0
    ...
    for relative, resource in iter_template_files():
        ...
        if destination.exists() and not args.force:
            continue
        copy_template_file(relative, resource, target)
    write_template_manifest(target, manifest)
    set_repo_root(target)
    reindex_status = run_reindex(check=False)
    if reindex_status != 0:
        return reindex_status
    print(f"Initialized Braingent memory repo at {target}")
    return 0
...
def update_plan_for_target(target: Path, manifest: dict[str, str], previous: dict[str, str]) -> dict[str, list[str]]:
    ...
    for relative, expected_hash in manifest.items():
        ...
        elif previous.get(relative) == current_hash:
...
def print_update_plan(plan: dict[str, list[str]]) -> None:
    for key in ("add", "update", "conflict", "unchanged"):
...
def cmd_update(args: argparse.Namespace) -> int:
    target = Path(args.target).expanduser().resolve()
    if not is_braingent_root(target):
        print(f"{target} does not look like a Braingent repo.", file=sys.stderr)
        return 1

    manifest = build_template_manifest()
    previous = read_existing_manifest(target)
    plan = update_plan_for_target(target, manifest, previous)
    print_update_plan(plan)

    if plan["conflict"] and not args.force:
        print("Conflicts detected. Re-run with --force to overwrite conflicted template files.", file=sys.stderr)
        return 1

    if args.dry_run or not args.write:
        print("Dry run only. Re-run with --write to apply non-conflicting updates.")
        return 0

    resources = dict(iter_template_files())
    for relative in [*plan["add"], *plan["update"], *(plan["conflict"] if args.force else [])]:
        copy_template_file(relative, resources[relative], target)
    write_template_manifest(target, manifest)
    set_repo_root(target)
    reindex_status = run_reindex(check=False)
    if reindex_status != 0:
        return reindex_status
    print(f"Updated Braingent memory repo at {target}")
    return 0


def cmd_qa_generate(args: argparse.Namespace) -> int:
    from braingent.qa import test_plan

    if args.qa_help:
        return test_plan.run(["--help"])
    return test_plan.run(args.qa_args)


def cmd_mcp_serve(args: argparse.Namespace) -> int:
    if args.mcp_root:
        set_repo_root(args.mcp_root)
    from braingent import mcp_server
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser = subparsers.add_parser("init", help="copy the starter template into a new memory repo")
    init_parser.add_argument("target", nargs="?", default=".", help="directory to initialize")
    init_parser.add_argument("--dry-run", action="store_true", help="show what would be copied")
    init_parser.add_argument("--force", action="store_true", help="allow copying into a non-empty target")
    init_parser.set_defaults(func=cmd_init)

    update_parser = subparsers.add_parser("update", help="conservatively update starter template files")
    update_parser.add_argument("target", nargs="?", default=".", help="Braingent repo to update")
    update_parser.add_argument("--dry-run", action="store_true", help="show the update plan without writing")
    update_parser.add_argument("--write", action="store_true", help="apply non-conflicting template updates")
    update_parser.add_argument("--force", action="store_true", help="overwrite conflicted template files when used with --write")
    update_parser.set_defaults(func=cmd_update)

    qa_parser = subparsers.add_parser("qa", help="QA helper commands")
    qa_subparsers = qa_parser.add_subparsers(dest="qa_command", required=True)
    qa_generate_parser = qa_subparsers.add_parser(
        "generate",
        add_help=False,
        help="generate a traceable QA test plan",
    )
    qa_generate_parser.add_argument("-h", "--help", action="store_true", dest="qa_help")
    qa_generate_parser.add_argument("qa_args", nargs=argparse.REMAINDER)
    qa_generate_parser.set_defaults(func=cmd_qa_generate)

    mcp_parser = subparsers.add_parser("mcp", help="MCP helper commands")
    mcp_subparsers = mcp_parser.add_subparsers(dest="mcp_command", required=True)
    mcp_serve_parser = mcp_subparsers.add_parser("serve", help="serve Braingent MCP tools over stdio")
    mcp_serve_parser.add_argument("--path", "--root", dest="mcp_root", help="Braingent repo root for MCP tools")
    ...
    find_parser.add_argument("filters", nargs="*", help="filters like kind=decision org=example topic=auth")
    ...
    task_new_parser.add_argument("--organization", default="org--personal")
    task_new_parser.add_argument("--project", default="project--personal--braingent")
