...
def _resolve_safe(path: str) -> Path:
    candidate = (braingent.REPO_ROOT / path).resolve()
...
def _record_from_index_row(row: sqlite3.Row) -> braingent.Record:
    ...
    return braingent.Record(path=braingent.REPO_ROOT / str(row["path"]), frontmatter=frontmatter, body="")
...
def guide() -> dict[str, Any]:
    ...
    for relpath in READ_FIRST_FILES:
        path = braingent.REPO_ROOT / relpath
        if not path.exists():
            continue
